...
import { addCustomer, changeCurrentItem } from '../../store/slices/customersSlice';
import { schemaCustomer } from '../../scemas';
...
const AddCustomer = () => {
  ...
  const { customersSlice } = useAppSelector(stateSelector);
  const currentData = customersSlice.currentData || { id: null };
  ...
  const myChange: SubmitHandler<IPostData> = (data) => {
    dispatch(changeCurrentItem(data));
  };
  ...
  return (
    <div className="flex flex-col w-[512px] gap-[20px] p-[40px] bg-#E2E8F0">
      ...
      <form className=" w-[100%]  flex flex-col gap-[20px]" onSubmit={handleSubmit(currentData.id ? myChange : mySubmit)}>
        ...
        {!currentData.id && <div className='flex flex-col gap-2 '>
